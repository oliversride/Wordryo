...
static void
and_draw_updateTrayButtons( DrawCtx* dctx, const XP_Rect* rect1, const XP_Rect* rect2, XP_U16 numInTray)
{
    DRAW_CBK_HEADER( "updateTrayButtons", "(Landroid/graphics/Rect;Landroid/graphics/Rect;I)V" );

    jobject jrect1 = makeJRect( draw, JCACHE_RECT0, rect1 );
    jobject jrect2 = makeJRect( draw, JCACHE_RECT1, rect2 );

    (*env)->CallVoidMethod( env, draw->jdraw, mid,
    						jrect1, jrect2, numInTray );
}
                      ...
                      const DictionaryCtxt* dict )
{
    ...
    if ( NULL != draw->jdraw ) {
        ...
        if ( code != draw->curLang ) {
        	...
        	//__android_log_print(ANDROID_LOG_INFO, "drawwrapper.c", "and_draw_dictChanged...env=%i mid=%i", env, mid);
            (*env)->CallVoidMethod( env, draw->jdraw, mid, (jint)dict );
        	//__android_log_print(ANDROID_LOG_INFO, "drawwrapper.c", "and_draw_dictChanged");
        }
    }
}
