...
import com.oliversride.wordryo.jni.JNIUtils;
import com.oliversride.wordryo.jni.JNIUtilsImpl;
import com.oliversride.wordryo.jni.TransportProcs;
import com.oliversride.wordryo.jni.UtilCtxt;
import com.oliversride.wordryo.jni.UtilCtxtImpl;
...
public class BoardActivity extends XWActivity 
    ...
    private static final int GAME_OVER_ASK_NEW = DLG_OKONLY + 15;
    private static final int GAME_DELETE = DLG_OKONLY + 16;
    ...
    protected Dialog onCreateDialog( final int id )
    {
        ...
        if ( null == dialog ) {
            ...
            switch ( id ) {
            case GAME_OVER_ASK_NEW:
            	// Game over, ask to start new game.
                ab = new AlertDialog.Builder( this )
                .setTitle( m_dlgTitle )
                .setMessage(m_dlgBytes + '\n' + getResources().getString(R.string.start_new_game) )
    ...
    public boolean onPrepareOptionsMenu( Menu menu ) 
    {
        ...
        if ( null != m_gsi ) {
            ...
            item = menu.findItem(R.id.board_menu_trade);
            item.setEnabled( !m_gameOver );
            ...
            if ( hintsOn || meVsAndroidGame() ){
            	item = menu.findItem( R.id.give_hint );
            ...
            final boolean allTilesInTray = !m_gsi.canUndo;
            if (allTilesInTray){
            	updateScorePending(0, -1);
            }
            
        }

        item = menu.findItem(R.id.zoom_plus_minus);
        if (JNIThread.ZOOM_BIGGER == JNIThread.smZoomDirection){
        	item.setIcon(R.drawable.zoom_plus);        		
        }
        if (JNIThread.ZOOM_SMALLER == JNIThread.smZoomDirection){
        	item.setIcon(R.drawable.zoom_minus);        		
        }
        
        if ( !inTrade ) {
            item = menu.findItem( R.id.commit );
            item.setEnabled( null == m_gsi || m_gsi.curTurnSelected );
        }

        if ( inTrade ){
            item = menu.findItem( R.id.go_back );
        	item.setVisible(false);
        	item = menu.findItem( R.id.board_menu_tray );
        	item.setVisible(false);
        	item = menu.findItem( R.id.give_hint );
        	item.setVisible(false);
        	item = menu.findItem( R.id.delete_game );
        	item.setVisible(false);
        }
...
//        }
    ...
    public boolean onOptionsItemSelected( MenuItem item ) 
    {
        ...
        case R.id.commit:
        	handleCommit();
        	break;
        case R.id.zoom_plus_minus:
        	if (JNIThread.ZOOM_BIGGER == JNIThread.smZoomDirection){
            	doZoom(2);        		
        	} else if (JNIThread.ZOOM_SMALLER == JNIThread.smZoomDirection){
            	doZoom(-2);        		
        	}
        ...
        case R.id.delete_game:
    		showConfirmThen( R.string.confirm_delete, GAME_DELETE );
        	break;
        case R.id.give_hint:
...
//            showNotAgainDlgThen( R.string.not_again_done, R.string.key_notagain_done, COMMIT_ACTION );
//            break;
//            
...
//            break;
    ...
    private void setGotGameDict( String getDict )
    {
        ...
        finish();
        GameUtils.launchGame( this, m_rowid, false, DBUtils.getGameType(this, m_rowid) );
    }
    ...
    private class BoardUtilCtxt extends UtilCtxtImpl {
        ...
        public void androidNoMove()
        {
            post( new Runnable() {
                    public void run() {
                        final String msg = getString( R.string.android_no_move );
                        Utils.showToast( BoardActivity.this, msg );
                    }
                } );
        }
        @Override
        public void androidExchangedTiles()
        {
            post( new Runnable() {
                    public void run() {
                        final String msg = getString( R.string.android_exchanged_tiles );
                        Utils.showToast( BoardActivity.this, msg );
                    }
                } );
        }
        ...
        public void noHintAvailable()
        {
            post( new Runnable() {
                    public void run() {
                        final String msg = getString( R.string.no_hint_available );
                        Utils.showToast( BoardActivity.this, msg );
                    }
                } );
        }
        ...
        {
        	if ( !delayUpdate ){
            	final int fnewTurn = newTurn;
                runOnUiThread(new Runnable(){
        			@Override
        			public void run() {
        		        updateTurnIndicator(fnewTurn);
        			}
                });        		
        	} else {
        		m_view.m_newTurn = newTurn;
        		m_view.m_delayTurnChange = delayUpdate;
        	}
            if ( 0 <= newTurn ) {
                post( new Runnable() {
                        public void run() {
        ...
        public void userError( int code )
        {
            ...
            case UtilCtxt.ERR_TWO_TILES_FIRST_MOVE:
                ...
                break;
            case UtilCtxt.ERR_MUST_START_ON_STAR:
                resid = R.string.str_must_start_on_star;
                break;
...
//            hideShowItem( menu, R.id.board_menu_chat, m_gsi.gameIsConnected );
    ...
    private void initScoreBoard(){
    	final int turn = DBUtils.getTurn(this, m_gameLock);
    	if (-1 == turn){
    		// Starting new game.
    	...
    	initForNumberOfPlayers(m_gi.nPlayers);
    }

    private void initForNumberOfPlayers(int numberOfPlayers){
    	final boolean squeeze = (3 == numberOfPlayers) || (4 == numberOfPlayers);
    	if ( squeeze ){
    		int textSize = 17;
    		View v = findViewById(R.id.board_view);
    		if ("layout-large".equals(v.getTag())){
    			textSize = 27;
    		}
    		((TextView) findViewById(R.id.score_player1_name)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player2_name)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player3_name)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player4_name)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player1_score)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player2_score)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player3_score)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player4_score)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player1_pending)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player2_pending)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player3_pending)).setTextSize(textSize);
    		((TextView) findViewById(R.id.score_player4_pending)).setTextSize(textSize);		
    	}
    	switch(numberOfPlayers){
    	case 3:
    		findViewById(R.id.score_player1_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player2_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player3_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player4_name).setVisibility(View.INVISIBLE);
    		findViewById(R.id.score_player1_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player2_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player3_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player4_score).setVisibility(View.INVISIBLE);
    		break;
    	case 4:
    		findViewById(R.id.score_player1_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player2_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player3_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player4_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player1_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player2_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player3_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player4_score).setVisibility(View.VISIBLE);
    		break;
    	default: // 2
    		findViewById(R.id.score_player1_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player2_name).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player3_name).setVisibility(View.GONE);
    		findViewById(R.id.score_player4_name).setVisibility(View.GONE);
    		findViewById(R.id.score_player1_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player2_score).setVisibility(View.VISIBLE);
    		findViewById(R.id.score_player3_score).setVisibility(View.GONE);
    		findViewById(R.id.score_player4_score).setVisibility(View.GONE);
    		break;
    	}    	
    }

    private boolean meVsAndroidGame(){
    	final Intent intent = getIntent();
    	final int gameType = intent.getIntExtra(GameUtils.INTENT_KEY_GAME_TYPE, -1);
    	final boolean meVsAndroid = (gameType == DBUtils.GAME_TYPE_ANDROID);
    	return meVsAndroid;
    }

	//
	// Start new Me vs. Android game.
	//
	private void startNewMeVsAndroid(){
		...
		GameUtils.launchGame( BoardActivity.this, rowid, false, DBUtils.GAME_TYPE_ANDROID );
	}

	private void startNewPassAndPlay(){
        final boolean hintsOn = CommonPrefs.getDefaultHintsAllowed(this, false);
    	final SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences( this );
    ...
    public void updateScoreBoard(DrawScoreInfo dsi){
    	View v;
    	if (0 == dsi.playerNum){
        	v = findViewById(R.id.score_player1_name);
        	((TextView) v).setText(dsi.name);
        	v = findViewById(R.id.score_player1_score);
        	((TextView) v).setText(String.valueOf(dsi.totalScore));
    	} else if (1 == dsi.playerNum){
        	v = findViewById(R.id.score_player2_name);
        	((TextView) v).setText(dsi.name);
        	v = findViewById(R.id.score_player2_score);
        	((TextView) v).setText(String.valueOf(dsi.totalScore));
    	} else if (2 == dsi.playerNum){
        	v = findViewById(R.id.score_player3_name);
        	((TextView) v).setText(dsi.name);
        	v = findViewById(R.id.score_player3_score);
        	((TextView) v).setText(String.valueOf(dsi.totalScore));
    	} else if (3 == dsi.playerNum){
        	v = findViewById(R.id.score_player4_name);
        	((TextView) v).setText(dsi.name);
        	v = findViewById(R.id.score_player4_score);
        	((TextView) v).setText(String.valueOf(dsi.totalScore));    		
    	}
    }

    public void updateScorePending(int score, int playerNum){
    	...
    	if (score <= -1){
        	...
        	pending = " +" + String.valueOf(score);
    	}
    	findViewById(R.id.score_player1_pending).setVisibility(View.INVISIBLE);
    	findViewById(R.id.score_player2_pending).setVisibility(View.INVISIBLE);
    	findViewById(R.id.score_player3_pending).setVisibility(View.INVISIBLE); 		
    	findViewById(R.id.score_player4_pending).setVisibility(View.INVISIBLE); 		
    	View v;
    	if (3 == playerNum){
        	v = findViewById(R.id.score_player4_pending);
        	v.setVisibility(View.VISIBLE);
        	((TextView) v).setText(pending);	
    	} else if (2 == playerNum){
        	v = findViewById(R.id.score_player3_pending);
        	v.setVisibility(View.VISIBLE);
        	((TextView) v).setText(pending);	    		
    	} else if (1 == playerNum){
        	v = findViewById(R.id.score_player2_pending);
        	v.setVisibility(View.VISIBLE);
        	((TextView) v).setText(pending);	    		
    	} else if (0 == playerNum){
        	v = findViewById(R.id.score_player1_pending);
        	v.setVisibility(View.VISIBLE);
        	((TextView) v).setText(pending);	    		    		
    	}
    }

    public void updateTurnIndicator(int newTurn){
    	if (null == m_gi) return; // Shouldn't happen but does.
    	final View v1 = findViewById(R.id.turn_indicator1);
    	final View v2 = findViewById(R.id.turn_indicator2);
    	final View v3 = findViewById(R.id.turn_indicator3);
    	final View v4 = findViewById(R.id.turn_indicator4);
