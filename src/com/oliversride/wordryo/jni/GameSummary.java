...
public class GameSummary {
	private static final String TAG = "GameSummary";
    ...
    // +W
    //
    private int gameType = DBUtils.GAME_TYPE_ANDROID;

    private boolean meVsAndroid(){
    	boolean meVsAndroid = false;
        for (int i = 0; i < nPlayers && !meVsAndroid; ++i) {
        	for(int j = 0; j < m_gi.nPlayers && !meVsAndroid; j++){
               	meVsAndroid = m_gi.players[j].isRobot();
        	}
        }
        return meVsAndroid;
    }
    ...
    public int getGameType(){
    	...
    	if ( !meVsAndroid() ){
    		type = DBUtils.GAME_TYPE_PASS_AND_PLAY;
    	}
