                                          ...
                                          GameLock lock )
    {
        ...
        synchronized( s_dbHelper ) {
            ...
            String[] columns = { ROW_ID,
                                 ...
                                 DBHelper.GAME_OVER, DBHelper.GAME_TYPE, DBHelper.PLAYERS,
                                    ...
                                    GameSummary summary, String inviteID )
    {
        ...
        synchronized( s_dbHelper ) {
            ...
            if ( null == summary ) {
                ...
                values.put( DBHelper.GAME_TYPE, summary.getGameType() );
...
//    public static boolean isSet( DBChangeListener listener ){
//        synchronized( s_listeners ) {
//            return s_listeners.contains( listener ) ;
//        }    	
//    }
    ...
    public static int getTurn( Context context, GameLock lock ){
        boolean[] isLocal = new boolean[1];
        int turn = -1;
    	GameSummary gs = DBUtils.getSummary(context, lock);
    	for(int i = 0; i < gs.nPlayers && -1 == turn; i++){
    		if (gs.isNextToPlay(i, isLocal)) turn = i;
    	}
    	return turn;
    }

    public static long[] getRowIDs(Context context){
        long[] result = null;
        initDB( context );
        synchronized( s_dbHelper ) {
            SQLiteDatabase db = s_dbHelper.getReadableDatabase();
    ...
    public static int getGameType( Context context, long rowid ) 
    {
        int gameType = getInt( context, rowid, DBHelper.GAME_TYPE, GAME_TYPE_ANDROID );
        
        return gameType;
    }

    public final static int GAME_TYPE_ANDROID = 1;
    public final static int GAME_TYPE_PASS_AND_PLAY = 2;
    public final static int GAME_TYPE_KIK = 3;
